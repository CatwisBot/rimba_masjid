...
import { getBeritaById, updateBerita } from "@/app/actions/berita";
    ...
    async function fetchArticle() {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          setError(res.error || "Berita tidak ditemukan.");
        }
      } catch {
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between gap-4 bg-surface p-6 rounded-3xl border border-border/80 shadow-2xs">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-black text-text">Edit Berita</h2>
          <p className="text-xs sm:text-sm text-text/70 mt-0.5">
            Perbarui informasi artikel atau publikasi kegiatan.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20 gap-3 text-text/60 bg-surface rounded-3xl border border-border">
          ...
          <span className="text-sm font-semibold">Memuat data berita...</span>
        </div>
      ) : (
        /* Form Container */
          ...
          <form onSubmit={handleSubmit} className="space-y-6 text-xs sm:text-sm">
            ...
            <div>
              ...
              <textarea
                rows={10}
                required
                placeholder="Tuliskan konten berita secara lengkap..."
